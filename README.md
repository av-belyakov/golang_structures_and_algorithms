# Golang structures and algorithms
...
- clickhousedb - Пример развёртывания СУБД Clickhouse в docker контейнере. Пример взаимодействия с СУБД Clickhouse.
...
- withshards - простое шардирование, где количество шардов вычисляется на основе остатка от деления хэшей
- consistenthashing - согласованное хеширование, распределённая схема хеширования, которая позволяет распределять данные между узлами кластера, минимизируя перераспределение ключей при добавлении или удалении узлов
...
##### algorithms - Примеры различных алгоритмов

- binarysearchalgorithm - алгоритм бинарного поиска;
- twopointeralgorithm - алгоритм двойного указателя (часто используется когда надо в срезе чисел найти два числа которые в сумме дают какое то определённое третье число):
...
- dijkstrasalgorithm - алгоритм Дейкстры, поиск кратчайшего пути в графе где рёбра имеют неотрицательные веса.
